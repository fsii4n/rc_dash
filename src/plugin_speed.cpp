// Speed plugin: km/h readout above the delta (kart use — secondary).
#include <stdio.h>
...
static void create(lv_obj_t *screen) {
  ...
  lv_obj_set_style_text_font(sLabel, &lv_font_montserrat_36, 0);
  lv_obj_set_style_text_color(sLabel, COL_TEXT, 0);
  lv_label_set_text(sLabel, "-- km/h");
  lv_obj_align(sLabel, LV_ALIGN_CENTER, 0, -174);
}
